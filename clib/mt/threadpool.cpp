...
c::threadpool::threadpool(size_t InitialWorkers, size_t MaxWorkers, const std::vector<size_t>& CoreBinding, const std::vector<size_t>& CoreExclude) :
...
void c::threadpool::stats(size_t& NumWorkers, size_t& NumAwaitingTasks, size_t& NumBusy) {
	{
		std::unique_lock<std::mutex> lock(PoolQueueSync);
...
void c::threadpool::check_resize(size_t NumWorker, size_t NumTasks) {
...
void c::threadpool::hire(size_t NumWorkers, WorkerType Type) {
	std::unique_lock<std::mutex> lock(PoolQueueSync);
	while (NumWorkers--)
	{
		std::thread ant([](
			...
			std::queue< std::function<void()> >& Tasks, threads& Workers, std::atomic_long& Busy, threadpool::WorkerType Type)
			{
				...
				if (core != -1) c::mt::set_thread_affinity(core);
